...
class ScoreController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $scores = $em->getRepository('TournoiBundle:Score')->findAll();
    ...
    {
        $score = new Score();
        $form = $this->createForm('GT\TournoiBundle\Form\ScoreType', $score);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('scores_show', array('id' => $score->getId()));
        }
    ...
    {
        $deleteForm = $this->createDeleteForm($score);
        $editForm = $this->createForm('GT\TournoiBundle\Form\ScoreType', $score);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('scores_edit', array('id' => $score->getId()));
        }
    ...
    private function createDeleteForm(Score $score)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('scores_delete', array('id' => $score->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
